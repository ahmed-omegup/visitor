...
public class LeafPathCollector {
    ...
    private void collect(Expression expression, String path, List<String> paths) {
        expression.accept(new Visitor<Void>() {
            public Void visit(Literal expression) { paths.add(path); return null; }
            public Void visit(VariableReference expression) { paths.add(path); return null; }
