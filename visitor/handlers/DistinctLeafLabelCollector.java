...
public class DistinctLeafLabelCollector {
    ...
    private void collect(Expression expression, Set<String> labels) {
        expression.accept(new Visitor<Void>() {
            public Void visit(Literal expression) { labels.add("literal:" + expression.value); return null; }
            public Void visit(VariableReference expression) { labels.add("variable:" + expression.name); return null; }
