...
public class UnaryNodeDepthSequenceBuilder extends AbstractExpressionFunction<List<Integer>> {
    UnaryNodeDepthSequenceBuilder() {}
    private int depth;
    ...
    public List<Integer> visit(Negation expression) { depths.add(depth); collect(expression.operand, depth + 1, depths); return null; }
    ...
    public List<Integer> visit(LogicalNot expression) { depths.add(depth); collect(expression.operand, depth + 1, depths); return null; }
