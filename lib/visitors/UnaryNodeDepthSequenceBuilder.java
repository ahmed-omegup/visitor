...
public class UnaryNodeDepthSequenceBuilder implements Visitor<List<Integer>> {
    ...
    public List<Integer> visit(Negation expression) {
            if (!active) { return handle(expression); } depths.add(depth); collect(expression.operand, depth + 1, depths); return null; }
