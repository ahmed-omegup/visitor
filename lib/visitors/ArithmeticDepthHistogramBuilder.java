...
public class ArithmeticDepthHistogramBuilder implements Visitor<Map<Integer, Integer>> {
    ...
    public Map<Integer, Integer> visit(Addition expression) {
            if (!active) { return handle(expression); } add(depth, histogram); collect(expression.left, depth + 1, histogram); collect(expression.right, depth + 1, histogram); return null; }
    ...
    public Map<Integer, Integer> visit(Division expression) {
            if (!active) { return handle(expression); } add(depth, histogram); collect(expression.dividend, depth + 1, histogram); collect(expression.divisor, depth + 1, histogram); return null; }
    ...
    public Map<Integer, Integer> visit(Exponentiation expression) {
            if (!active) { return handle(expression); } add(depth, histogram); collect(expression.base, depth + 1, histogram); collect(expression.exponent, depth + 1, histogram); return null; }
    ...
    public Map<Integer, Integer> visit(LogicalNot expression) {
            if (!active) { return handle(expression); } collect(expression.operand, depth + 1, histogram); return null; }
