...
public class VariableNameLengthHistogramBuilder extends AbstractExpressionFunction<Map<Integer, Integer>> {
    ...
    public Map<Integer, Integer> visit(VariableReference expression) { histogram.put(expression.name.length(), histogram.getOrDefault(expression.name.length(), 0) + 1); return null; }
