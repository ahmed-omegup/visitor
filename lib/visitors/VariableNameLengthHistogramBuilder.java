...
public class VariableNameLengthHistogramBuilder implements Visitor<Map<Integer, Integer>> {
    ...
    public Map<Integer, Integer> visit(VariableReference expression) {
            if (!active) { return handle(expression); } histogram.put(expression.name.length(), histogram.getOrDefault(expression.name.length(), 0) + 1); return null; }
