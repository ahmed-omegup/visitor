...
public class BinaryNodeDepthSequenceBuilder implements Visitor<List<Integer>> {
    ...
    public List<Integer> visit(Addition expression) {
            if (!active) { return handle(expression); } depths.add(depth); collect(expression.left, depth + 1, depths); collect(expression.right, depth + 1, depths); return null; }
    ...
    public List<Integer> visit(Division expression) {
            if (!active) { return handle(expression); } depths.add(depth); collect(expression.dividend, depth + 1, depths); collect(expression.divisor, depth + 1, depths); return null; }
    ...
    public List<Integer> visit(Exponentiation expression) {
            if (!active) { return handle(expression); } depths.add(depth); collect(expression.base, depth + 1, depths); collect(expression.exponent, depth + 1, depths); return null; }
