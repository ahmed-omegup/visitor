...
public class ConditionalPathCollector extends AbstractExpressionFunction<List<String>> {
    ConditionalPathCollector() {}
    private String path;
    ...
    public List<String> visit(Conditional expression) { paths.add(path);
        collect(expression.condition, path + ".condition", paths);
        collect(expression.whenTrue, path + ".whenTrue", paths);
        collect(expression.whenFalse, path + ".whenFalse", paths);
        return null;
    }
    public List<String> visit(FunctionCall expression) { collect(expression.callee, path + ".callee", paths);
