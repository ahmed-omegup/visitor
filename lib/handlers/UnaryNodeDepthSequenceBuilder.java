...
public class UnaryNodeDepthSequenceBuilder {
    ...
    private void collect(Expression expression, int depth, List<Integer> depths) {
        expression.accept(new Visitor<Void>() {
            ...
            public Void visit(Negation expression) { depths.add(depth); collect(expression.operand, depth + 1, depths); return null; }
            ...
            public Void visit(LogicalNot expression) { depths.add(depth); collect(expression.operand, depth + 1, depths); return null; }
