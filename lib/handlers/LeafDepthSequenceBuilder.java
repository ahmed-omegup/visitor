...
public class LeafDepthSequenceBuilder {
    ...
    private void collect(Expression expression, int depth, List<Integer> depths) {
        expression.accept(new Visitor<Void>() {
            public Void visit(Literal expression) { depths.add(depth); return null; }
            public Void visit(VariableReference expression) { depths.add(depth); return null; }
