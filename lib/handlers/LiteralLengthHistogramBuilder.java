...
public class LiteralLengthHistogramBuilder {
    ...
    private void collect(Expression expression, Map<Integer, Integer> histogram) {
        expression.accept(new Visitor<Void>() {
            public Void visit(Literal expression) { histogram.put(expression.value.length(), histogram.getOrDefault(expression.value.length(), 0) + 1); return null; }
