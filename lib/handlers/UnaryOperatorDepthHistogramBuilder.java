...
public class UnaryOperatorDepthHistogramBuilder {
    ...
    private void collect(Expression expression, int depth, Map<Integer, Integer> histogram) {
        expression.accept(new Visitor<Void>() {
            ...
            public Void visit(Negation expression) { add(depth, histogram); collect(expression.operand, depth + 1, histogram); return null; }
            ...
            public Void visit(LogicalNot expression) { add(depth, histogram); collect(expression.operand, depth + 1, histogram); return null; }
